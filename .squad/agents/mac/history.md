...
- **Preload bridge typed returns:** Replaced all `Promise<unknown>` return types in SquadAPI interface with proper `Promise<IpcResult<T>>` types imported from `main/types.ts`. Widened `tsconfig.preload.json` rootDir from `./src/preload` to `./src` to allow cross-boundary imports. Push channel callbacks also typed with `ConnectionState`, `SquadEvent`, `StreamDelta`, `UsageEvent` instead of `unknown`. Single source of truth — no type duplication between main and preload.
